...
export default function JarvisChat() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window)) {
      ...
      recognition.continuous = false  // И��менено на false
  ...
  const sendMessage = (message: string) => {
    ...
    setTimeout(() => {
      const jarvisResponses = [
        ...
        'Прекрасно, что обратились! У нас есть готовые решен��я для любого бизнеса. Расскажите о целях, подберу что-то идеальное. Говорю медленно для вашего комфорта.',
